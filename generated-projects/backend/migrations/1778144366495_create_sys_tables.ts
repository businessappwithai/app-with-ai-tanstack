 ...
 *
 * Generated: 2026-05-07T08:59:26.496Z
 */
...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('sys_ref_list', (table) => {
    ...
    table.integer('sys_reference_id').notNullable()
      .references('sys_reference_id').inTable('sys_reference').onDelete('CASCADE');
  ...
  await knex.schema.createTable('sys_column', (table) => {
    ...
    table.uuid('sys_table_id').notNullable()
      .references('sys_table_id').inTable('sys_table').onDelete('CASCADE');
    table.string('column_name', 100).notNullable();
    table.string('name', 100).notNullable();
    table.text('description');
    table.integer('sys_reference_id').notNullable()
      .references('sys_reference_id').inTable('sys_reference');
    table.uuid('sys_val_rule_id')
      .references('sys_val_rule_id').inTable('sys_val_rule').onDelete('SET NULL');
  ...
  await knex.schema.createTable('sys_ref_table', (table) => {
    ...
    table.uuid('sys_table_id').notNullable()
      .references('sys_table_id').inTable('sys_table').onDelete('CASCADE');
    table.uuid('key_column_id').notNullable()
      .references('sys_column_id').inTable('sys_column').onDelete('CASCADE');
    table.uuid('display_column_id').notNullable()
      .references('sys_column_id').inTable('sys_column').onDelete('CASCADE');
  ...
  await knex.schema.alterTable('sys_table', (table) => {
    table.foreign('sys_window_id').references('sys_window_id').inTable('sys_window').onDelete('SET NULL');
    table.foreign('po_window_id').references('sys_window_id').inTable('sys_window').onDelete('SET NULL');
  });
  ...
  await knex.schema.createTable('sys_tab', (table) => {
    ...
    table.uuid('sys_window_id').notNullable()
      .references('sys_window_id').inTable('sys_window').onDelete('CASCADE');
    table.uuid('sys_table_id').notNullable()
      .references('sys_table_id').inTable('sys_table').onDelete('CASCADE');
    table.string('name', 100).notNullable();
    table.text('description');
    ...
    table.uuid('parent_column_id')
      .references('sys_column_id').inTable('sys_column').onDelete('SET NULL');
    table.uuid('link_column_id')
      .references('sys_column_id').inTable('sys_column').onDelete('SET NULL');
    table.string('order_by_clause', 255);
    table.text('where_clause');
  ...
  await knex.schema.createTable('sys_field', (table) => {
    ...
    table.uuid('sys_tab_id').notNullable()
      .references('sys_tab_id').inTable('sys_tab').onDelete('CASCADE');
    table.uuid('sys_column_id').notNullable()
      .references('sys_column_id').inTable('sys_column').onDelete('CASCADE');
    table.uuid('sys_field_group_id')
      .references('sys_field_group_id').inTable('sys_field_group').onDelete('SET NULL');
    table.string('name', 100).notNullable();
    table.text('description');
    table.text('help');
    ...
    // Related components
    table.uuid('included_tab_id')
      .references('sys_tab_id').inTable('sys_tab').onDelete('SET NULL');
    table.string('default_value', 255);
  ...
  await knex.schema.createTable('sys_user', (table) => {
    ...
    table.uuid('supervisor_id')
      .references('sys_user_id').inTable('sys_user').onDelete('SET NULL');
    table.uuid('default_sys_role_id')
      .references('sys_role_id').inTable('sys_role').onDelete('SET NULL');
    table.string('entity_type', 40).notNullable().defaultTo('U');
    table.boolean('is_active').notNullable().defaultTo(true);
    table.string('created_by', 100).notNullable();
    table.string('updated_by', 100).notNullable();
  ...
  await knex.schema.createTable('sys_user_roles', (table) => {
    ...
    table.uuid('sys_user_id').notNullable()
      .references('sys_user_id').inTable('sys_user').onDelete('CASCADE');
    table.uuid('sys_role_id').notNullable()
      .references('sys_role_id').inTable('sys_role').onDelete('CASCADE');
    table.string('entity_type', 40).notNullable().defaultTo('U');
    table.boolean('is_active').notNullable().defaultTo(true);
    table.string('created_by', 100).notNullable();
    table.string('updated_by', 100).notNullable();
  ...
  await knex.schema.createTable('sys_change_log', (table) => {
    ...
    table.uuid('sys_table_id').notNullable()
      .references('sys_table_id').inTable('sys_table').onDelete('CASCADE');
    table.uuid('sys_column_id')
      .references('sys_column_id').inTable('sys_column').onDelete('SET NULL');
    table.string('record_id', 36).notNullable();
    table.uuid('sys_user_id')
      .references('sys_user_id').inTable('sys_user').onDelete('SET NULL');
