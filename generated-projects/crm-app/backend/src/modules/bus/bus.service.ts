...
import { Injectable, NotFoundException, BadRequestException, ConflictException, Inject, Logger, Optional } from '@nestjs/common';
...
export class BusService {
  ...
  async findById(entity: string, id: string): Promise<any> {
    ...
    const result = this.db
      ...
      : await this.knex(tableName).where('id', processedId.id || id).whereNull('deleted_at').first();
  ...
  async update(
    ...
    if (this.db) {
      ...
      // Always include version increment and timestamp even for empty patches
