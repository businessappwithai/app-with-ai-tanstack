...
export const Route = createFileRoute('/bus_activity/$id')({
  ...
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<Activity>) => {
      if (isNew) {
        ...
        return apiClient.patch<Activity>(
          `/api/bus/bus_activity/${id}`,
          formData,
          {
    ...
    onSuccess: (savedData) => {
      ...
      if (isNew) {
        navigate({ to: '/bus_activity/$id', params: { id: String(savedData['id']) } });
      } else {
