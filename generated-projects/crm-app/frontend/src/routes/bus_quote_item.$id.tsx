 ...
 *
 * Generated: 2026-05-12T11:57:05.741Z
 * Project: crm-app
 */
...
export const Route = createFileRoute('/bus_quote_item/$id')({
  ...
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<QuoteItem>) => {
      if (isNew) {
        ...
        return apiClient.patch<QuoteItem>(
          `/api/bus/bus_quote_item/${id}`,
          formData,
          {
    ...
    onSuccess: (savedData) => {
      ...
      if (isNew) {
        navigate({ to: '/bus_quote_item/$id', params: { id: String(savedData['id']) } });
      } else {
