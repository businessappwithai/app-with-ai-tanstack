...
export function useEntities<T extends EntityRecord = EntityRecord>(
  ...
  return useQuery<PaginatedResponse<T>, ApiError>({
    ...
    queryFn: () => apiClient.get<PaginatedResponse<T>>(`/api/bus/${entity}`, params),
    ...options,
  });
}
...
export function useEntity<T extends EntityRecord = EntityRecord>(
  ...
  return useQuery<T, ApiError>({
    ...
    queryFn: () => apiClient.get<T>(`/api/bus/${entity}/${id}`),
    enabled: !!id,
    ...options,
  });
}
...
export function useEntityMetadata(entity: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<any>(`/api/bus/${entity}/meta`);
      return response;
    },
...
export function useTableMetadata(entity: string) {
  return useQuery<TableMetadata>({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get<{ data: TableMetadata[] }>(`/api/sys/tables`);
...
export function useFormFields(entity: string) {
  return useQuery<FieldMetadata[]>({
    ...
    queryFn: async () => {
      const response = await apiClient.get<FieldMetadata[]>(`/api/bus/${entity}/fields/form`);
...
export function useGridFields(entity: string) {
  return useQuery<FieldMetadata[]>({
    ...
    queryFn: async () => {
      const response = await apiClient.get<FieldMetadata[]>(`/api/bus/${entity}/fields/grid`);
      return Array.isArray(response) ? response : ((response as any).data || []);
    },
    staleTime: 30 * 60 * 1000,
  });
  ...
  return useQuery<RefListValue[]>({
    ...
    queryFn: async () => {
      const response = await apiClient.get<{ data: RefListValue[] }>(
        `/api/sys/ref-list?sys_reference_id=${sysReferenceId}`
      );
...
export function useAllFormFields(entity: string) {
  return useQuery<FieldMetadata[]>({
    ...
    queryFn: async () => {
      const response = await apiClient.get<FieldMetadata[]>(`/api/sys/fields/form?entity=${entity}`);
      return Array.isArray(response) ? response : ((response as any).data || []);
    },
    staleTime: 30 * 60 * 1000,
  });
...
export function useAllGridFields(entity: string) {
  return useQuery<FieldMetadata[]>({
    ...
    queryFn: async () => {
      const response = await apiClient.get<FieldMetadata[]>(`/api/sys/fields/grid?entity=${entity}`);
      return Array.isArray(response) ? response : ((response as any).data || []);
    },
    staleTime: 30 * 60 * 1000,
  });
...
export function useUpdateFieldStyle(entity: string) {
  ...
  return useMutation({
    mutationFn: async ({ fieldId, style }: { fieldId: string; style: Partial<FieldMetadata> }) => {
      return apiClient.patch(`/api/sys/fields/${fieldId}?entity=${entity}`, style);
    },
    onSuccess: () => {
...
export function useCreateEntity<T extends EntityRecord = EntityRecord>(
  ...
  return useMutation<T, ApiError, Partial<T>, MutationContext>({
    mutationFn: (data) => apiClient.post<T>(`/api/bus/${entity}`, data),
    ...
    onError: (err, newData, context) => {
      ...
      toast.error(translate('entities.createFailed' as any, { error: errorMessage }));
    },
    onSettled: () => {
...
export function useUpdateEntity<T extends EntityRecord = EntityRecord>(
  ...
  return useMutation<T, ApiError, MutationVariables, MutationContext>({
    mutationFn: ({ id, data, version }) =>
      apiClient.patch<T>(`/api/bus/${entity}/${id}`, data, {
    ...
    onError: (err, variables, context) => {
      ...
      const errorMessage = Array.isArray(err.message) ? err.message.join(', ') : err.message;
      toast.error(translate('entities.updateFailed' as any, { error: errorMessage }));
    },
...
export function useDeleteEntity(entity: string) {
  ...
  return useMutation<void, ApiError, string, MutationContext>({
    mutationFn: (id) => apiClient.delete(`/api/bus/${entity}/${id}`),
    onMutate: async (id) => {
      // Cancel outgoing refetches
      await queryClient.cancelQueries({ queryKey: entityKeys.detail(entity, id) });
      await queryClient.cancelQueries({ queryKey: entityKeys.lists() });
    ...
    onError: (err, id, context) => {
      ...
      queryClient.setQueryData(entityKeys.lists(), context?.previousList);
      const errorMessage = Array.isArray(err.message) ? err.message.join(', ') : err.message;
      toast.error(translate('entities.deleteFailed' as any, { error: errorMessage }));
    },
    onSettled: () => {
      // Refetch to ensure server state
      queryClient.invalidateQueries({ queryKey: entityKeys.lists() });
...
export function usePrefetchEntity(entity: string) {
  ...
  return (id: string) => {
    queryClient.prefetchQuery({
      queryKey: entityKeys.detail(entity, id),
      queryFn: () => apiClient.get(`/api/bus/${entity}/${id}`),
    });
  };
}
...
export function usePrefetchEntityList(entity: string) {
  ...
  return (params?: EntityListParams) => {
    queryClient.prefetchQuery({
      queryKey: entityKeys.list(entity, params),
      queryFn: () => apiClient.get(`/api/bus/${entity}`, params),
    });
  };
}
  ...
  return useQuery<FieldGroup[]>({
    ...
    queryFn: async () => {
      const response = await apiClient.get<{ data: FieldGroup[] }>(
        '/api/sys/field-groups?entity=' + entity
      );
      return response.data || [];
    },
    staleTime: 5 * 60 * 1000,
...
export function useCreateFieldGroup(entity: string) {
  ...
  return useMutation<FieldGroup, ApiError, Partial<FieldGroup>>({
    mutationFn: (data) =>
      apiClient.post<FieldGroup>('/api/sys/field-groups?entity=' + entity, data),
    onSuccess: () => {
  ...
  >({
    mutationFn: ({ id, data }) =>
      apiClient.put<FieldGroup>(
        '/api/sys/field-groups/' + id + '?entity=' + entity,
        data
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: fieldGroupKeys.list(entity) });
...
export function useDeleteFieldGroup(entity: string) {
  ...
  return useMutation<void, ApiError, string>({
    mutationFn: (id) =>
      apiClient.delete('/api/sys/field-groups/' + id + '?entity=' + entity),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: fieldGroupKeys.list(entity) });
