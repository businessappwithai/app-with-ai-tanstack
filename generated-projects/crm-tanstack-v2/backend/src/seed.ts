 ...
 *
 * Generated: 2026-05-15T16:06:25.867Z
 */
...
async function createDialect(): Promise<PostgresDialect | SqliteDialect> {
  if (dbClient === 'better-sqlite3' || dbClient === 'sqlite') {
    ...
    const dbPath = process.env.DATABASE_FILENAME ?? path.join(__dirname, '..', 'data', 'c_r_m _app.db');
    ...
    return new PostgresDialect({
      pool: new Pool({
        ...
        database: process.env.DATABASE_NAME ?? 'c_r_m _app',
      }),
    });
  }
}
