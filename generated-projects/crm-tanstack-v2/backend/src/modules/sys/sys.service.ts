 ...
 *
 * Generated: 2026-05-15T16:06:25.846Z
 */
...
export class SysService {
  ...
  async findAllDatabaseTables(options: PaginationOptions & { search?: string } = {}) {
    ...
    // Query information_schema via raw SQL (Kysely doesn't type information_schema)
    let whereClause = `table_schema = 'public' AND table_type = 'BASE TABLE'`;
    if (prefix) whereClause += ` AND table_name LIKE '${prefix}%'`;
    if (search) whereClause += ` AND table_name LIKE '%${search}%'`;
