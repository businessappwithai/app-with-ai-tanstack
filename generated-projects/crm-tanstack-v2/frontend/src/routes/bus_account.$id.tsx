 ...
 *
 * Displays and allows editing of a single ACCOUNT record
...
export const Route = createFileRoute('/bus_account/$id')({
...
function EntityDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields('bus_account');

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['account', id],
    queryFn: () => apiClient.get<ACCOUNT>(`/api/bus/bus_account/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<ACCOUNT>) => {
      if (isNew) {
        return apiClient.post<ACCOUNT>('/api/bus/bus_account', formData);
      } else {
        return apiClient.patch<ACCOUNT>(
          `/api/bus/bus_account/${id}`,
          formData,
          {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success('ACCOUNT ' + (isNew ? 'created' : 'updated') + ' successfully');
      if (isNew) {
        navigate({ to: '/bus_account/$id', params: { id: String(savedData['id']) } });
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_account/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['account'] });
      navigate({ to: '/bus_account' });
    },
    onError: () => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading ACCOUNT</p>
