 ...
 *
 * Displays and allows editing of a single OPPORTUNITY record
 ...
 *
 * Generated: 2026-05-15T16:06:27.790Z
 * Project: CRM App
 */
...
export const Route = createFileRoute('/bus_opportunity/$id')({
...
function EntityDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields('bus_opportunity');

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['opportunity', id],
    queryFn: () => apiClient.get<OPPORTUNITY>(`/api/bus/bus_opportunity/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<OPPORTUNITY>) => {
      if (isNew) {
        return apiClient.post<OPPORTUNITY>('/api/bus/bus_opportunity', formData);
      } else {
        return apiClient.patch<OPPORTUNITY>(
          `/api/bus/bus_opportunity/${id}`,
          formData,
          {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success('OPPORTUNITY ' + (isNew ? 'created' : 'updated') + ' successfully');
      if (isNew) {
        navigate({ to: '/bus_opportunity/$id', params: { id: String(savedData['id']) } });
      } else {
    ...
    onError: async (error: any) => {
      ...
      let message = 'Failed to save OPPORTUNITY';
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_opportunity/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['opportunity'] });
      navigate({ to: '/bus_opportunity' });
    },
    onError: () => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading OPPORTUNITY</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link to="/bus_opportunity">
