...
test.describe("EmailTemplate CRUD Tests", () => {
  const entityName = "EmailTemplate";
  const entityNamePlural = "bus_email_template";
  ...
  test("should display EmailTemplate list", async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid="entity-table"] tbody tr:not([data-testid="loading-row"])', {
        timeout: 10000,
      })
      .catch(() => {
  ...
  test("should create a new EmailTemplate", async ({ page }) => {
    ...
    await page
      .waitForSelector("form .animate-pulse", { state: "detached", timeout: 5000 })
      .catch(() => {
    ...
    const firstInput = page
      .locator(
        'input:not([type="hidden"]):not([style*="display: none"]), textarea:not([style*="display: none"]), select:not([style*="display: none"])'
      )
      .first();
    ...
    await expect(
      page
        ...
        .or(page.locator(`h1:has-text("${entityName}")`))
    ).toBeVisible({ timeout: 10000 });
  });

  test("should view EmailTemplate details", async ({ page }) => {
    // Click on first item in list using data-testid
    const table = page.getByTestId("entity-table");
    await table
      .locator('tbody tr:first-child a, tbody tr:first-child button:has-text("View")')
      .first()
      .click();
  ...
  test("should update a EmailTemplate", async ({ page }) => {
    ...
    if (!isEnabled) {
      ...
      const allInputs = page.locator(
        'input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]):not([style*="display: none"]), textarea:not([style*="display: "none"]), select:not([style*="display: "none"])'
      );
    ...
    // Verify success
    await expect(page.locator("text=updated, text=success, text=Success").first()).toBeVisible({
      timeout: 5000,
    });
  });

  test("should delete a EmailTemplate", async ({ page }) => {
    ...
    const confirmButton = page
      .locator(
        'button:has-text("Delete Permanently"), button:has-text("Confirm"), button:has-text("Yes"), button:has-text("Delete")'
      )
      .first();
