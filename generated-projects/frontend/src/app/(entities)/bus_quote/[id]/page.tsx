 ...
 *
 * Displays and allows editing of a single Quote record
...
export default function QuoteDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_quote");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["quote", id],
    queryFn: () => apiClient.get<Quote>(`/api/bus/bus_quote/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<Quote>) => {
      if (isNew) {
        return apiClient.post<Quote>("/api/bus/bus_quote", formData);
      } else {
        return apiClient.patch<Quote>(`/api/bus/bus_quote/${id}`, formData, {
    ...
    onSuccess: (savedData) => {
      ...
      if (isNew) {
        router.push("/bus_quote/" + savedData.id);
      } else {
    ...
    onError: async (error: any) => {
      ...
      let message = "Failed to save Quote";
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_quote/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["quote"] });
      // Toast is shown by dynamic-table
      router.push("/bus_quote");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading Quote</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-foreground" data-testid="entity-heading">
                    {isNew ? "Create Quote" : mode === "edit" ? "Edit Quote" : "Quote Detail"}
                  </h1>
                  {!isNew && data && (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    This action cannot be undone. This will permanently delete the Quote record.
                  </p>
                </div>
              </div>
