/**
 * EmailTemplate Detail/Edit Page
 *
 * Displays and allows editing of a single EmailTemplate record
 ...
 *
 * Generated: 2026-05-07T09:31:28.787Z
 * Project: crm-app
 */
...
export default function EmailTemplateDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_email_template");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["email-template", id],
    queryFn: () => apiClient.get<EmailTemplate>(`/api/bus/bus_email_template/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<EmailTemplate>) => {
      if (isNew) {
        return apiClient.post<EmailTemplate>("/api/bus/bus_email_template", formData);
      } else {
        return apiClient.patch<EmailTemplate>(`/api/bus/bus_email_template/${id}`, formData, {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success("EmailTemplate " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_email_template/" + savedData.id);
      } else {
    ...
    onError: async (error: any) => {
      ...
      let message = "Failed to save EmailTemplate";
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_email_template/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["email-template"] });
      // Toast is shown by dynamic-table
      router.push("/bus_email_template");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading EmailTemplate</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_email_template">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    This action cannot be undone. This will permanently delete the EmailTemplate
                    record.
                  </p>
                </div>
              </div>
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="rounded-xl border border-border/60 bg-gradient-to-br from-muted/40 to-muted/20 backdrop-blur-sm p-5 shadow-sm">
              ...
              <div className="flex flex-col gap-2">
                <Link href="/bus_email_template/new">
                  <Button
                    variant="outline"
                    size="sm"
                ...
                <Link href="/bus_email_template">
                  <Button
                    ...
                    className="w-full justify-start shadow-sm hover:shadow-md transition-shadow"
                  >
                    View All EmailTemplate Records
                  </Button>
                </Link>
                <Link href="/dashboard">
                  <Button
