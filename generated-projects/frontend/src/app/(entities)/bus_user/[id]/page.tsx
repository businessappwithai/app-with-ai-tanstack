 ...
 *
 * Displays and allows editing of a single User record
 ...
 *
 * Generated: 2026-05-07T09:31:28.795Z
 * Project: crm-app
 */
...
export default function UserDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_user");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["user", id],
    queryFn: () => apiClient.get<User>(`/api/bus/bus_user/${id}`),
    enabled: !isNew && !!id,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<User>) => {
      if (isNew) {
        return apiClient.post<User>("/api/bus/bus_user", formData);
      } else {
        return apiClient.patch<User>(`/api/bus/bus_user/${id}`, formData, {
    ...
    onSuccess: (savedData) => {
      ...
      if (isNew) {
        router.push("/bus_user/" + savedData.id);
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_user/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["user"] });
      // Toast is shown by dynamic-table
      router.push("/bus_user");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading User</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-foreground" data-testid="entity-heading">
                    {isNew ? "Create User" : mode === "edit" ? "Edit User" : "User Detail"}
                  </h1>
                  {!isNew && data && (
