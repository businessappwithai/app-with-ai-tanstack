 ...
 *
 * Displays and allows editing of a single Deal record
 ...
 *
 * Generated: 2026-05-07T09:31:28.771Z
 * Project: crm-app
 */
...
export default function DealDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_deal");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["deal", id],
    queryFn: () => apiClient.get<Deal>(`/api/bus/bus_deal/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<Deal>) => {
      if (isNew) {
        return apiClient.post<Deal>("/api/bus/bus_deal", formData);
      } else {
        return apiClient.patch<Deal>(`/api/bus/bus_deal/${id}`, formData, {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success("Deal " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_deal/" + savedData.id);
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_deal/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["deal"] });
      // Toast is shown by dynamic-table
      router.push("/bus_deal");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading Deal</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-foreground" data-testid="entity-heading">
                    {isNew ? "Create Deal" : mode === "edit" ? "Edit Deal" : "Deal Detail"}
                  </h1>
                  {!isNew && data && (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    This action cannot be undone. This will permanently delete the Deal record.
                  </p>
                </div>
              </div>
