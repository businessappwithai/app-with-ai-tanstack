 ...
 *
 * Displays and allows editing of a single Activity record
 ...
 *
 * Generated: 2026-05-07T09:31:28.779Z
 * Project: crm-app
 */
...
export default function ActivityDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_activity");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["activity", id],
    queryFn: () => apiClient.get<Activity>(`/api/bus/bus_activity/${id}`),
    enabled: !isNew && !!id,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<Activity>) => {
      if (isNew) {
        return apiClient.post<Activity>("/api/bus/bus_activity", formData);
      } else {
        return apiClient.patch<Activity>(`/api/bus/bus_activity/${id}`, formData, {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success("Activity " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_activity/" + savedData.id);
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_activity/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["activity"] });
      // Toast is shown by dynamic-table
      router.push("/bus_activity");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading Activity</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_activity">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    This action cannot be undone. This will permanently delete the Activity record.
                  </p>
                </div>
              </div>
