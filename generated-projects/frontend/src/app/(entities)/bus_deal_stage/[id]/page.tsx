/**
 * DealStage Detail/Edit Page
 *
 * Displays and allows editing of a single DealStage record
 ...
 *
 * Generated: 2026-05-07T09:31:28.773Z
 * Project: crm-app
 */
...
export default function DealStageDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_deal_stage");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["deal-stage", id],
    queryFn: () => apiClient.get<DealStage>(`/api/bus/bus_deal_stage/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<DealStage>) => {
      if (isNew) {
        return apiClient.post<DealStage>("/api/bus/bus_deal_stage", formData);
      } else {
        return apiClient.patch<DealStage>(`/api/bus/bus_deal_stage/${id}`, formData, {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success("DealStage " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_deal_stage/" + savedData.id);
      } else {
    ...
    onError: async (error: any) => {
      ...
      let message = "Failed to save DealStage";
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_deal_stage/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["deal-stage"] });
      // Toast is shown by dynamic-table
      router.push("/bus_deal_stage");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading DealStage</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_deal_stage">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    This action cannot be undone. This will permanently delete the DealStage record.
                  </p>
                </div>
              </div>
