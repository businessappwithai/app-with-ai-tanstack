/**
 * QuoteItem Detail/Edit Page
 *
 * Displays and allows editing of a single QuoteItem record
 ...
 *
 * Generated: 2026-05-07T09:31:28.793Z
 * Project: crm-app
 */
...
export default function QuoteItemDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_quote_item");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["quote-item", id],
    queryFn: () => apiClient.get<QuoteItem>(`/api/bus/bus_quote_item/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<QuoteItem>) => {
      if (isNew) {
        return apiClient.post<QuoteItem>("/api/bus/bus_quote_item", formData);
      } else {
        return apiClient.patch<QuoteItem>(`/api/bus/bus_quote_item/${id}`, formData, {
          headers: data?.version ? { "If-Match": `"${data.version}"` } : {},
        });
      }
    },
    onSuccess: (savedData) => {
      queryClient.invalidateQueries({ queryKey: ["quote-item"] });
      setServerErrors({}); // Clear errors on success
      toast.success("QuoteItem " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_quote_item/" + savedData.id);
      } else {
    ...
    onError: async (error: any) => {
      ...
      let message = "Failed to save QuoteItem";
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_quote_item/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["quote-item"] });
      // Toast is shown by dynamic-table
      router.push("/bus_quote_item");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading QuoteItem</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_quote_item">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    This action cannot be undone. This will permanently delete the QuoteItem record.
                  </p>
                </div>
              </div>
