 ...
 *
 * Displays and allows editing of a single Contact record
 ...
 *
 * Generated: 2026-05-07T09:31:28.770Z
 * Project: crm-app
 */
...
export default function ContactDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_contact");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["contact", id],
    queryFn: () => apiClient.get<Contact>(`/api/bus/bus_contact/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<Contact>) => {
      if (isNew) {
        return apiClient.post<Contact>("/api/bus/bus_contact", formData);
      } else {
        return apiClient.patch<Contact>(`/api/bus/bus_contact/${id}`, formData, {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success("Contact " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_contact/" + savedData.id);
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_contact/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contact"] });
      // Toast is shown by dynamic-table
      router.push("/bus_contact");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading Contact</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_contact">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-foreground" data-testid="entity-heading">
                    {isNew ? "Create Contact" : mode === "edit" ? "Edit Contact" : "Contact Detail"}
                  </h1>
                  {!isNew && data && (
