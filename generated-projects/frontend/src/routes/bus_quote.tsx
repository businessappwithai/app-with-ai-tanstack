 ...
 * Quote List Page
 *
 * Displays a paginated, sortable list of Quote records
...
export const Route = createFileRoute('/bus_quote')({
...
function EntityListPage() {
  ...
  const { data: gridFields } = useGridFields('bus_quote')
  ...
  const {
    ...
    queryKey: ['quote', 'list', page, pageSize, filterParams],
    queryFn: () => {
      ...
      return apiClient.get<PaginatedResponse<Quote>>(
        `/api/bus/bus_quote?${params.toString()}`,
      )
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/bus/bus_quote/${id}`)
    },
    onSuccess: () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto py-8 px-4 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">Quote entity</p>
                </div>
              </div>
            </div>
        ...
        <DynamicTable
          ...
          onView={(id) => navigate({ to: '/bus_quote/$id', params: { id } })}
          onEdit={(id) => navigate({ to: '/bus_quote/$id', params: { id } })}
          onDelete={(id) => {
