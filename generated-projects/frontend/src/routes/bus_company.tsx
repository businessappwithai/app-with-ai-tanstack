 ...
 * Company List Page
 *
 * Displays a paginated, sortable list of Company records
 ...
 *
 * Generated: 2026-05-31T11:58:04.527Z
 * Project: crm-app
 */
...
export const Route = createFileRoute('/bus_company')({
...
function EntityListPage() {
  ...
  const { data: gridFields } = useGridFields('bus_company')
  ...
  const {
    ...
    queryKey: ['company', 'list', page, pageSize, filterParams],
    queryFn: () => {
      ...
      return apiClient.get<PaginatedResponse<Company>>(
        `/api/bus/bus_company?${params.toString()}`,
      )
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/bus/bus_company/${id}`)
    },
    onSuccess: () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto py-8 px-4 space-y-6">
        ...
        <DynamicTable
          ...
          onView={(id) => navigate({ to: '/bus_company/$id', params: { id } })}
          onEdit={(id) => navigate({ to: '/bus_company/$id', params: { id } })}
          onDelete={(id) => {
