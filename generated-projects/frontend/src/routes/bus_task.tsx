 ...
 * Task List Page
 *
 * Displays a paginated, sortable list of Task records
...
export const Route = createFileRoute('/bus_task')({
...
function EntityListPage() {
  ...
  const { data: gridFields } = useGridFields('bus_task')
  ...
  const {
    ...
    queryKey: ['task', 'list', page, pageSize, filterParams],
    queryFn: () => {
      ...
      return apiClient.get<PaginatedResponse<Task>>(
        `/api/bus/bus_task?${params.toString()}`,
      )
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/bus/bus_task/${id}`)
    },
    onSuccess: () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto py-8 px-4 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">Task entity</p>
                </div>
              </div>
            </div>
        ...
        <DynamicTable
          ...
          onView={(id) => navigate({ to: '/bus_task/$id', params: { id } })}
          onEdit={(id) => navigate({ to: '/bus_task/$id', params: { id } })}
          onDelete={(id) => {
