...
export const Route = createFileRoute('/bus_quote/$id')({
  ...
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<Quote>) => {
      if (isNew) {
        ...
        return apiClient.patch<Quote>(
          `/api/bus/bus_quote/${id}`,
          formData,
          {
    ...
    onSuccess: (savedData) => {
      ...
      if (isNew) {
        navigate({ to: '/bus_quote/$id', params: { id: String(savedData['id']) } });
      } else {
