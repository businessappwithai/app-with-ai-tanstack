 ...
 * User List Page
 *
 * Displays a paginated, sortable list of User records
...
export const Route = createFileRoute('/bus_user')({
...
function EntityListPage() {
  ...
  const { data: gridFields } = useGridFields('bus_user')
  ...
  const {
    ...
    queryKey: ['user', 'list', page, pageSize, filterParams],
    queryFn: () => {
      ...
      return apiClient.get<PaginatedResponse<User>>(
        `/api/bus/bus_user?${params.toString()}`,
      )
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/bus/bus_user/${id}`)
    },
    onSuccess: () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto py-8 px-4 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">User entity</p>
                </div>
              </div>
            </div>
        ...
        <DynamicTable
          ...
          onView={(id) => navigate({ to: '/bus_user/$id', params: { id } })}
          onEdit={(id) => navigate({ to: '/bus_user/$id', params: { id } })}
          onDelete={(id) => {
