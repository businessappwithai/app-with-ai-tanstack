/**
 * EmailMessage Detail/Edit Page
 *
 * Displays and allows editing of a single EmailMessage record
 ...
 *
 * Generated: 2026-05-31T11:58:04.536Z
 * Project: crm-app
 */
...
export const Route = createFileRoute('/bus_email_message/$id')({
...
function EntityDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields('bus_email_message');

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['email-message', id],
    queryFn: () => apiClient.get<EmailMessage>(`/api/bus/bus_email_message/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<EmailMessage>) => {
      if (isNew) {
        return apiClient.post<EmailMessage>('/api/bus/bus_email_message', formData);
      } else {
        return apiClient.patch<EmailMessage>(
          `/api/bus/bus_email_message/${id}`,
          formData,
          {
    ...
    onSuccess: (savedData) => {
      ...
      toast.success('EmailMessage ' + (isNew ? 'created' : 'updated') + ' successfully');
      if (isNew) {
        navigate({ to: '/bus_email_message/$id', params: { id: String(savedData['id']) } });
      } else {
    ...
    onError: async (error: any) => {
      ...
      let message = 'Failed to save EmailMessage';
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_email_message/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['email-message'] });
      navigate({ to: '/bus_email_message' });
    },
    onError: () => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading EmailMessage</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link to="/bus_email_message">
