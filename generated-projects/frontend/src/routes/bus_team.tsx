 ...
 * Team List Page
 *
 * Displays a paginated, sortable list of Team records
...
export const Route = createFileRoute('/bus_team')({
...
function EntityListPage() {
  ...
  const { data: gridFields } = useGridFields('bus_team')
  ...
  const {
    ...
    queryKey: ['team', 'list', page, pageSize, filterParams],
    queryFn: () => {
      ...
      return apiClient.get<PaginatedResponse<Team>>(
        `/api/bus/bus_team?${params.toString()}`,
      )
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/bus/bus_team/${id}`)
    },
    onSuccess: () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto py-8 px-4 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">Team entity</p>
                </div>
              </div>
            </div>
        ...
        <DynamicTable
          ...
          onView={(id) => navigate({ to: '/bus_team/$id', params: { id } })}
          onEdit={(id) => navigate({ to: '/bus_team/$id', params: { id } })}
          onDelete={(id) => {
