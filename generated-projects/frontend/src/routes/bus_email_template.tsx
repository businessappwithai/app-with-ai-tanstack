/**
 * EmailTemplate List Page
 *
 * Displays a paginated, sortable list of EmailTemplate records
...
export const Route = createFileRoute('/bus_email_template')({
  component: EntityListPageRoute,
})
...
function EntityListPage() {
  ...
  const { data: gridFields } = useGridFields('bus_email_template')
  ...
  const filterParams = useMemo(
    () => buildFilterParams(advancedFilters),
    [advancedFilters],
  )
  ...
  const {
    ...
    queryKey: ['email-template', 'list', page, pageSize, filterParams],
    queryFn: () => {
      ...
      return apiClient.get<PaginatedResponse<EmailTemplate>>(
        `/api/bus/bus_email_template?${params.toString()}`,
      )
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/api/bus/bus_email_template/${id}`)
    },
    onSuccess: () => {
  ...
  const addFilter = useCallback(() => {
    ...
    const defaultOps = gridFields
      ? getOperatorsForField(
          gridFields.find((f) => f.column_name === defaultField),
        )
      : TEXT_OPERATORS
        ...
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">
              Error loading EmailTemplate records
            </p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto py-8 px-4 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">EmailTemplate entity</p>
                </div>
              </div>
            </div>
              ...
              {advancedFilters.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    No filters added. Click &quot;Add Filter&quot; to build a
                    search query.
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <Select
                          value={filter.field}
                          onValueChange={(value) => {
                            const newField = gridFields?.find(
        ...
        <DynamicTable
          ...
          isLoading={isLoading}
          totalCount={
            quickSearch ? filteredData.length : response?.meta?.total || 0
          }
          onView={(id) =>
            navigate({ to: '/bus_email_template/$id', params: { id } })
          }
          onEdit={(id) =>
            navigate({ to: '/bus_email_template/$id', params: { id } })
          }
