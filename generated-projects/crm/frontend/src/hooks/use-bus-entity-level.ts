...
export function useBusEntityLevel(entityName: string) {
    ...
    queries: [
      ...
      },
      {
        ...
        queryFn: async () => {
          ...
          const table = tableResp.data.find((t: BusTableMeta) =>
            tableNameCandidates.includes(t.table_name)
          );
  ...
  const isLoading = formQuery.isLoading || gridQuery.isLoading || windowQuery.isLoading;
  ...
  const windowMeta = windowQuery.data as
    | { label: string; windowId?: string; windowSlug: string }
    | undefined;
