 ...
 * Two modes:
 *
 *   node cleanup.ts            Delete exactly the rows recorded in
 ...
 *
 *   node cleanup.ts --all      Delete every row in every bus_ table,
 ...
 *
 * Generated: 2026-08-29T04:45:22.099Z
 * Project: my-app
 */

import { login } from "./harness/auth.ts";
import { config } from "./harness/config.ts";
import { entities, topologicalEntities } from "./harness/entities.ts";
...
import { isServerUp, waitForServer } from "./harness/server.ts";
...
async function main(): Promise<void> {
  console.log("\n═══════════════════════════════════════════");
  console.log("  my-app — E2E data cleanup");
  console.log("═══════════════════════════════════════════\n");

  if (!(await isServerUp())) {
    console.error(
      `✗ No backend listening on ${config.baseUrl}.\n` +
        `  Start it first: cd ../backend && npm run start`
    );
    process.exit(1);
  }
  await waitForServer();
