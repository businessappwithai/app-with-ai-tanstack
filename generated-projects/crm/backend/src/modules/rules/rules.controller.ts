...
export class RulesController {
  ...
  // NOTE: static routes must be declared before ':id' or they get shadowed
  @Get('entities')
  ...
  getEntitiesWithRules() {
    return {
      entities: [
        ...
        { entityType: 'bus_territory', entityName: 'Territory' },
        { entityType: 'bus_account', entityName: 'Account' },
        { entityType: 'bus_contact', entityName: 'Contact' },
        { entityType: 'bus_lead', entityName: 'Lead' },
        { entityType: 'bus_campaign', entityName: 'Campaign' },
        { entityType: 'bus_campaign_member', entityName: 'CampaignMember' },
        { entityType: 'bus_opportunity', entityName: 'Opportunity' },
        { entityType: 'bus_opportunity_line_item', entityName: 'OpportunityLineItem' },
        { entityType: 'bus_product', entityName: 'Product' },
        { entityType: 'bus_quote', entityName: 'Quote' },
        { entityType: 'bus_quote_line_item', entityName: 'QuoteLineItem' },
        { entityType: 'bus_contract', entityName: 'Contract' },
        { entityType: 'bus_support_case', entityName: 'SupportCase' },
        { entityType: 'bus_sla_policy', entityName: 'SlaPolicy' },
  ...
  async create(
    @Body(new ZodValidationPipe(CreateRuleSchema)) dto: {
      entityName: string;
      ruleName: string;
      operation: string;
      jdmContent: string;
  ...
  async update(
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateRuleSchema)) dto: {
      jdmContent?: string;
      isActive?: boolean;
    },
    @CurrentUser() user: { id: string },
  ...
  async validate(@Body(new ZodValidationPipe(ValidateJdmSchema)) dto: { jdmContent: string }) {
  ...
  async dryRun(@Body(new ZodValidationPipe(DryRunSchema)) dto: { ruleId: string; testData: Record<string, unknown> }) {
    const rule = await this.rulesService.findById(dto.ruleId);
    return await this.rulesService.dryRun(rule.jdmContent, dto.testData);
  }
  ...
  async evaluateRules(
    @Body(new ZodValidationPipe(EvaluateRulesSchema))
    body: {
      entityName: string;
