 ...
 *
 * Generated: 2026-08-17T16:41:43.502Z
 */
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // Add foreign keys from sys_table to sys_window (idempotent: catches constraint-already-exists)
  await sql`
  ...
  // Add foreign key from sys_field_group to sys_tab (idempotent: catches constraint-already-exists)
  await sql`
  ...
  // ============================================================================
  // sys_element - Application Dictionary Element (domain-level column definition)
  // ============================================================================
  await sql`
  ...
  // ============================================================================
  // sys_rule_definitions - Business Rules (GoRules JDM decision models)
  // ============================================================================
  await sql`
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_sys_rule_definitions_entity ON sys_rule_definitions (entity_name, operation, is_active)`.execute(db);
}

export async function down(db: Kysely<any>): Promise<void> {
  // Drop in reverse order of creation
  await sql`DROP TABLE IF EXISTS sys_element CASCADE`.execute(db);
  await sql`DROP TABLE IF EXISTS sys_rule_definitions CASCADE`.execute(db);
