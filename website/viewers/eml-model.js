...
// Source: packages/generator/src/browser/viewers.ts
...
`);
    ...
    for (let i = 0;i < lines.length; i++) {
      ...
      if (trimmed.startsWith("%%")) {
        ...
        const fieldHelp2 = this.parseFieldHelpDirective(trimmed);
        if (fieldHelp2)
          fieldHelpText.push(fieldHelp2);
...
`)) {
      ...
      if (!line.startsWith("%%step"))
        continue;
      const autoType = line.match(AUTO_TYPE_DIRECTIVE2);
      ...
      const autoProp = line.match(AUTO_PROP_DIRECTIVE2);
    ...
    for (const nodeId of autoNodes) {
      ...
      if (step)
        step.props = sagaPropsFromAutomation2(step.type, step.props);
    }
...
// packages/generator/src/viewers/view-model.ts
var META = /^%%meta\s+(name|version|description)\s*:\s*(.+)$/;
...
function roleFromShape(shape, hasOutgoing) {
  switch (shape) {
    case "stadium":
...
function missingProperties(step) {
  ...
  if (!contract)
    return [];
  const missing2 = contract.required.filter((key) => !step.props[key]?.trim());
  for (const group of contract.oneOf ?? []) {
    if (!group.some((key) => step.props[key]?.trim()))
...
function readModel(source) {
  const text = source ?? "";
  const warnings = [];
  const parsed = parseModelQuietly(text, warnings);
  ...
  const rules = sections.map((section) => {
    ...
    return {
      ...
      tableName: compiled?.tableName ?? toTableName2(section.entity),
      nodes,
      edges,
  ...
  const workflowSections = extractWorkflowSections(text);
  ...
  const workflows = parsed.workflows.map((workflow) => {
  ...
  const sagas = parsed.sagas.map((saga) => ({
    ...saga,
...
function parseModelQuietly(source, into) {
  const original = console.warn;
  console.warn = (...args) => {
    into.push(args.map(String).join(" ").replace(/^\s*⚠️\s*/u, "").trim());
...
// packages/generator/src/browser/viewers.ts
setLanguageDefinition(appwithai_language_default);
function inspectModel(source) {
