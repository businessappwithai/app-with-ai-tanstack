/**
 * C U S T O M E R OData Controller
 * Table: bus_customer
 *
...
export class CUSTOMERController extends ODataController {
  ...
  /**
   * GET /odata/CUSTOMERs
   * List all C U S T O M E R records with OData query support
   */
  @odata.GET
  async getAll(
    @odata.query query: any = {},
    @odata.context ctx: ODataHttpContext
  ): Promise<ODataResult<CUSTOMER>> {
   ...
   * Get a single C U S T O M E R by ID
   */
  @odata.GET
  async getOne(
   ...
   * Create a new C U S T O M E R
   */
  @odata.POST
  async create(
    @odata.body body: Partial<CUSTOMER>,
    @odata.context ctx: ODataHttpContext
  ): Promise<CUSTOMER> {
    // Generate ID if not provided
    const recordBody = body as Record<string, unknown>;
    if (!recordBody[CUSTOMERController.primaryKey]) {
      recordBody[CUSTOMERController.primaryKey] = uuidv4();
    }
  ...
  /**
   * PUT /odata/CUSTOMERs(:key)
   * Full update of a C U S T O M E R
   */
  @odata.PUT
  async fullUpdate(
    @odata.key key: string,
    ...
    await getKnex()(CUSTOMERController.tableName)
      .where(CUSTOMERController.primaryKey, key)
      .update(updated);
  ...
  /**
   * PATCH /odata/CUSTOMERs(:key)
   * Partial update of a C U S T O M E R
   */
  @odata.PATCH
  async partialUpdate(
    @odata.key key: string,
    @odata.body delta: Partial<CUSTOMER>,
    @odata.context ctx: ODataHttpContext
  ): Promise<CUSTOMER> {
    const existing = await getKnex()(CUSTOMERController.tableName)
      .where(CUSTOMERController.primaryKey, key)
  ...
  /**
   * DELETE /odata/CUSTOMERs(:key)
   * Soft delete a C U S T O M E R
   */
  @odata.DELETE
