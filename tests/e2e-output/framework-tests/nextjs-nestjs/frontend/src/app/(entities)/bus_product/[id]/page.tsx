 ...
 *
 * Displays and allows editing of a single PRODUCT record
...
export default function PRODUCTDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_product");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["p-r-o-d-u-c-t", id],
    queryFn: () => apiClient.get<PRODUCT>(`/api/bus/bus_product/${id}`),
    enabled: !isNew && !!id,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<PRODUCT>) => {
      if (isNew) {
        return apiClient.post<PRODUCT>("/api/bus/bus_product", formData);
      } else {
        return apiClient.patch<PRODUCT>(`/api/bus/bus_product/${id}`, formData, {
          headers: data?.version ? { "If-Match": `"${data.version}"` } : {},
        });
      }
    },
    onSuccess: (savedData) => {
      queryClient.invalidateQueries({ queryKey: ["p-r-o-d-u-c-t"] });
      setServerErrors({}); // Clear errors on success
      toast.success("PRODUCT " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_product/" + savedData.id);
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_product/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["p-r-o-d-u-c-t"] });
      // Toast is shown by dynamic-table
      router.push("/bus_product");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading PRODUCT</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_product">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-foreground" data-testid="entity-heading">
                    {isNew ? "Create PRODUCT" : mode === "edit" ? "Edit PRODUCT" : "PRODUCT Detail"}
                  </h1>
                  {!isNew && data && (
