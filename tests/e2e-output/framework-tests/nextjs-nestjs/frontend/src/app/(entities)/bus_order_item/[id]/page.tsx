/**
 * ORDER_ITEM Detail/Edit Page
 *
 * Displays and allows editing of a single ORDER_ITEM record
 ...
 *
 * Generated: 2026-03-20T16:41:26.616Z
...
export default function ORDER_ITEMDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_order_item");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["o-r-d-e-r-i-t-e-m", id],
    queryFn: () => apiClient.get<ORDER_ITEM>(`/api/bus/bus_order_item/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<ORDER_ITEM>) => {
      if (isNew) {
        return apiClient.post<ORDER_ITEM>("/api/bus/bus_order_item", formData);
      } else {
        return apiClient.patch<ORDER_ITEM>(`/api/bus/bus_order_item/${id}`, formData, {
          headers: data?.version ? { "If-Match": `"${data.version}"` } : {},
        });
      }
    },
    onSuccess: (savedData) => {
      queryClient.invalidateQueries({ queryKey: ["o-r-d-e-r-i-t-e-m"] });
      setServerErrors({}); // Clear errors on success
      toast.success("ORDER_ITEM " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_order_item/" + savedData.id);
      } else {
    ...
    onError: async (error: any) => {
      ...
      let message = "Failed to save ORDER_ITEM";
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_order_item/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["o-r-d-e-r-i-t-e-m"] });
      // Toast is shown by dynamic-table
      router.push("/bus_order_item");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading ORDER_ITEM</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_order_item">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    This action cannot be undone. This will permanently delete the ORDER_ITEM
                    record.
                  </p>
                </div>
              </div>
