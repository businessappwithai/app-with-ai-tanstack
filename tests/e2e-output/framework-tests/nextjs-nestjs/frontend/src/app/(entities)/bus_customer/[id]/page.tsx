 ...
 *
 * Displays and allows editing of a single CUSTOMER record
 ...
 *
 * Generated: 2026-03-20T16:41:26.614Z
...
export default function CUSTOMERDetailPage() {
  ...
  const { data: formFields, isLoading: isLoadingFields } = useFormFields("bus_customer");

  // Fetch the record data
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["c-u-s-t-o-m-e-r", id],
    queryFn: () => apiClient.get<CUSTOMER>(`/api/bus/bus_customer/${id}`),
    enabled: !isNew && !!id,
  });

  // Save mutation (create or update)
  const saveMutation = useMutation({
    mutationFn: async (formData: Partial<CUSTOMER>) => {
      if (isNew) {
        return apiClient.post<CUSTOMER>("/api/bus/bus_customer", formData);
      } else {
        return apiClient.patch<CUSTOMER>(`/api/bus/bus_customer/${id}`, formData, {
          headers: data?.version ? { "If-Match": `"${data.version}"` } : {},
        });
      }
    },
    onSuccess: (savedData) => {
      queryClient.invalidateQueries({ queryKey: ["c-u-s-t-o-m-e-r"] });
      setServerErrors({}); // Clear errors on success
      toast.success("CUSTOMER " + (isNew ? "created" : "updated") + " successfully");
      if (isNew) {
        router.push("/bus_customer/" + savedData.id);
      } else {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return apiClient.delete(`/api/bus/bus_customer/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["c-u-s-t-o-m-e-r"] });
      // Toast is shown by dynamic-table
      router.push("/bus_customer");
    },
    onError: () => {
      // Toast is shown by dynamic-table
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto py-8 px-4">
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border/60 bg-muted/30 p-12 text-center backdrop-blur-sm">
            ...
            <p className="text-destructive text-lg font-semibold">Error loading CUSTOMER</p>
            <p className="text-sm text-muted-foreground mt-2">
              The record may not exist or the server may be unavailable.
            </p>
            <div className="flex gap-3 mt-6">
              <Link href="/bus_customer">
