...
test.describe.serial("@browser generating and running the CRM", () => {
  let page: Page;
  let problems: string[];
...
test.describe.serial("@browser an uploaded model", () => {
  let page: Page;
  ...
  test("a chosen file is read in the tab and named from its filename", async () => {
    await page
      .locator("#file")
      .setInputFiles(new URL("../../../examples/drug-discovery.eml.mmd", import.meta.url).pathname);
