...
console.log("content width:", await page.evaluate(() => document.querySelector(".content").getBoundingClientRect().width));
...
const info = await frame.evaluate(() => {
  ...
  if (!el) return { found: false, ids: [...document.querySelectorAll(".field__input")].map(e=>e.id).slice(0,6) };
  const r = el.getBoundingClientRect();
  const cs = getComputedStyle(el);
  return { found: true, rect: { x: r.x, y: r.y, w: r.width, h: r.height }, display: cs.display, visibility: cs.visibility, disabled: el.disabled, docH: document.documentElement.scrollHeight, winH: window.innerHeight };
});
console.log("field-name:", JSON.stringify(info));
try {
  await frame.fill("#field-name", "Probe Co", { timeout: 12000 });
  console.log("fill: OK");
} catch (e) { console.log("fill FAILED:", e.message.split("\n").slice(0,4).join(" | ")); }
