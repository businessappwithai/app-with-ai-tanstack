...
import { FullStackGenerator, StackOption } from '../generators/full-stack.generator';
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (isMultiFileMode && options.input) {
        throw new Error('Cannot use both --input and separate file options (--sys-file, --bus-file, --ref-file). Use one or the other.');
      }

      if (!isMultiFileMode && !options.input) {
        throw new Error('Either --input or at least one of --sys-file, --bus-file, --ref-file must be specified.');
      }
      ...
      if (isMultiFileMode) {
        ...
        if (options.sysFile) {
          ...
          const { entities: sysEntities, relationships: sysRelationships } = parser.parse(sysContent);
        ...
        if (options.busFile) {
          ...
          const { entities: busEntities, relationships: busRelationships } = parser.parse(busContent);
        ...
        if (options.refFile) {
          ...
          const { entities: refEntities, relationships: refRelationships } = parser.parse(refContent);
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      const description = await question('Description [Generated application]: ') || 'Generated application';
