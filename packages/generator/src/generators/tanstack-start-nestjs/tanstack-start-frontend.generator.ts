...
export class TanStackStartFrontendGenerator extends BaseGenerator {
  ...
  private async generateComponents(outputDir: string, _context: any): Promise<void> {
    ...
    // Copy routeTree.gen.ts (pre-generated by TanStack Router; rebuilt on first dev run)
    try {
      await fs.copyFile(
        path.join(templateDir, "src/routeTree.gen.ts"),
        path.join(outputDir, "src/routeTree.gen.ts")
      );
    } catch (e) {
      // Optional — TanStack Router regenerates this on dev/build
    }
  }
