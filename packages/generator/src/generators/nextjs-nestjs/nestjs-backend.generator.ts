...
import { entityToBusEntity, generateEntityDictionary } from '@erdwithai/core/types';
...
export class NestJsBackendGenerator extends BaseGenerator {
  ...
  private prepareContext(entities: Entity[], relationships: Relationship[]): Record<string, unknown> {
    ...
    const sysColumns = dictionaryEntries.map((entry, entityIndex) => {
    ...
    const sysFields = dictionaryEntries.map((entry, entityIndex) => {
    ...
    const dbUser = this.options.databaseType === 'postgresql'
  ...
  private async generateCoreFiles(outputDir: string, context: any): Promise<void> {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/decorators/public.decorator.ts'), publicDecoratorContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/decorators/roles.decorator.ts'), rolesDecoratorContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/decorators/current-user.decorator.ts'), currentUserDecoratorContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/guards/jwt-auth.guard.ts'), jwtGuardContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/guards/session-auth.guard.ts'), sessionAuthGuardContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/guards/roles.guard.ts'), rolesGuardContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/auth.controller.ts'), authControllerContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/modules/auth/auth.module.ts'), authModuleContent);
    } catch (e) {
    ...
    // Job queue module
    const jobQueueFiles = [
      { tpl: 'src/modules/jobs/job-queue.module.ts.hbs', out: 'src/modules/jobs/job-queue.module.ts' },
      { tpl: 'src/modules/jobs/job-queue.service.ts.hbs', out: 'src/modules/jobs/job-queue.service.ts' },
      { tpl: 'src/modules/jobs/job-queue.controller.ts.hbs', out: 'src/modules/jobs/job-queue.controller.ts' },
    ];
    ...
    const rulesFiles = [
      ...
      { tpl: 'src/modules/rules/rules.controller.ts.hbs', out: 'src/modules/rules/rules.controller.ts' },
      { tpl: 'src/modules/rules/rules-engine.service.ts.hbs', out: 'src/modules/rules/rules-engine.service.ts' },
    ];
    ...
    // Workflow module
    const workflowFiles = [
      { tpl: 'src/modules/workflow/workflow.module.ts.hbs', out: 'src/modules/workflow/workflow.module.ts' },
      { tpl: 'src/modules/workflow/workflow.service.ts.hbs', out: 'src/modules/workflow/workflow.service.ts' },
      { tpl: 'src/modules/workflow/workflow.controller.ts.hbs', out: 'src/modules/workflow/workflow.controller.ts' },
    ];
    ...
    for (const entity of context.entities) {
      try {
        ...
        const jdmContent = await this.renderTemplate('src/modules/rules/jdm/entity.jdm.json.hbs', entityContext);
        await fs.writeFile(path.join(outputDir, `src/modules/rules/jdm/${entity.tableName}.jdm.json`), jdmContent);
      } catch (e) {
  ...
  private async generateSysTables(outputDir: string, context: any): Promise<void> {
    ...
    await fs.writeFile(path.join(outputDir, 'src/modules/sys/sys.controller.ts'), sysControllerContent);
...
`;
    ...
    await fs.writeFile(path.join(outputDir, 'src/modules/bus/bus.controller.ts'), controllerContent);
  ...
  private async generateMigrations(outputDir: string, context: any): Promise<void> {
    ...
    const busMigrationContent = await this.renderTemplate('../../common/migrations/bus-tables.migration.ts.hbs', context);
    await fs.writeFile(
  ...
  private async generateConfigFiles(outputDir: string, context: any): Promise<void> {
    ...
    await fs.writeFile(path.join(outputDir, 'src/database/database.constants.ts'), dbConstantsContent);

    // Database service
    try {
      const dbServiceContent = await this.renderTemplate('src/database/database.service.ts.hbs', context);
      await fs.writeFile(path.join(outputDir, 'src/database/database.service.ts'), dbServiceContent);
    } catch (e) {
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'src/database/database.service.decorator.ts'), dbServiceDecoratorContent);
    } catch (e) {
  ...
  private async generateTestFiles(outputDir: string, context: any): Promise<void> {
    ...
    // Auth tests
    const authTestFiles = [
      { tpl: 'test/modules/auth/auth.controller.test.ts.hbs', out: 'test/modules/auth/auth.controller.test.ts' },
      { tpl: 'test/modules/auth/jwt-auth.guard.test.ts.hbs', out: 'test/modules/auth/jwt-auth.guard.test.ts' },
    ];
    ...
    try {
      ...
      await fs.writeFile(path.join(outputDir, 'test/modules/jobs/job-queue.service.test.ts'), jobQueueTestContent);
    } catch (e) {
