...
export class BusController {
  ...
  async findAll(
    ...
    this.logger.debug(
      `[${methodName}] Request - entity: ${entity}, page: ${page}, limit: ${limit}, orderBy: ${orderBy}, orderDir: ${orderDir}, search: ${search}`
    );
    ...
    const result = await this.busService.findAll(
      entity,
      { page, limit, orderBy, orderDir, search, searchFields },
      parsedFilters
    );
  ...
  async update(
    ...
    if (req) {
      try {
        req.__auditBefore = await this.busService.findById(entity, id);
      } catch {
        /* not found is ok */
      }
    }
