...
export class BpmnExecutorService {
  ...
  async execute(bpmnXml: string, ctx: WorkflowContext): Promise<BpmnExecutionResult> {
    ...
    try {
      ...
      this.logger.log(
        `BPMN executor: ${tasks.length} task(s) for ${ctx.entityName}:${ctx.entityId}`
      );
  ...
  private parseTask(raw: any): BpmnTask {
    ...
    for (const e of extArr) {
      const propList =
        e["erdwithai:properties"]?.["erdwithai:property"] ?? e["erdwithai:property"] ?? [];
  ...
  private async executeNode(task: BpmnTask, ctx: WorkflowContext): Promise<void> {
    ...
    switch (task.nodeType) {
      case "UpdateEntity":
        return this.execUpdateEntity(task, ctx);
      case "CreateEntity":
        return this.execCreateEntity(task, ctx);
      case "Formula":
        return this.execFormula(task, ctx);
      case "REST":
        return this.execRest(task, ctx);
      case "Agent":
        return this.execAgent(task, ctx);
      default:
  ...
  private async execUpdateEntity(task: BpmnTask, ctx: WorkflowContext): Promise<void> {
    ...
    const tableName = entity
      ? entity.startsWith("bus_")
        ? entity
        : `bus_${entity}`
      : `bus_${ctx.entityName.replace(/^bus_/, "")}`;

    const resolvedValue =
      source && ctx.decision[source] !== undefined
        ? ctx.decision[source]
        : source && ctx.vars[source] !== undefined
          ? ctx.vars[source]
          : value;
    ...
    if (fields) {
      try {
        ...
        for (const [k, v] of Object.entries(parsed)) {
          fieldMap[k] =
            ctx.decision[v] !== undefined
              ? ctx.decision[v]
              : ctx.vars[v] !== undefined
                ? ctx.vars[v]
                : ctx.entityData[v] !== undefined
                  ? ctx.entityData[v]
                  : v;
        }
      } catch {
  ...
  private execFormula(task: BpmnTask, ctx: WorkflowContext): Promise<void> {
    ...
    switch (operation) {
      case "multiply":
        result = baseVal * operandVal;
        break;
      case "divide":
        result = operandVal !== 0 ? baseVal / operandVal : 0;
        break;
      case "add":
        result = baseVal + operandVal;
        break;
