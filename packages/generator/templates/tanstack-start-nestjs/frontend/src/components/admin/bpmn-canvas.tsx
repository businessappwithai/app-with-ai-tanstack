...
>(({ className, entityName }, ref) => {
  ...
  const addNodeType = useCallback((nodeType: NodeType) => {
    ...
    const propObjects = [
      moddle.create("erdwithai:Property", { name: "nodeType", value: nodeType }),
    ];
    ...
    <div className={`flex gap-4 h-full ${className ?? ""}`}>
      ...
      <div className="flex-1 relative border rounded-lg overflow-hidden bg-white">
        ...
        <div className="absolute bottom-2 left-2 text-xs text-gray-400 pointer-events-none">
          Click a node button → or drag ServiceTask from palette → connect with arrows → select to
          configure
        </div>
      </div>
              ...
              {NODE_TYPES.map((t) => (
                ...
                >
                  ...
                  <span>
                    ...
                    <span className="block text-gray-400 text-[10px] leading-tight mt-0.5">
                      {NODE_TYPE_DESC[t]}
                    </span>
                  </span>
                </button>
    ...
    return (
      <div className="space-y-3">
        ...
        <p className="text-xs text-amber-600">
          Agent invocation is currently a placeholder pending Mastra integration.
        </p>
      </div>
    );
  }
