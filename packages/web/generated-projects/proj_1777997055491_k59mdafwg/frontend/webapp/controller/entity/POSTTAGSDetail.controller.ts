/**
 * POSTTAGS Detail Controller - Object Page (Column 3)
 *
 * Dedicated detail controller for Post Tags entity.
 ...
 */
export default class POSTTAGSDetail extends Controller {
    ...
    try {
      oModel
        ...
        .then(() => {
          oViewModel.setProperty("/busy", false);
          oViewModel.setProperty("/editable", false);
          MessageHelper.showSuccess("Post Tags saved successfully");
        })
        .catch((oError: any) => {
          oViewModel.setProperty("/busy", false);
  ...
  public onDeletePress(): void {
    MessageBox.confirm("Are you sure you want to delete this Post Tags?", {
  ...
  private _deleteRecord(): void {
    ...
    oModel
      ...
      .then(() => {
        oViewModel.setProperty("/busy", false);
        MessageHelper.showSuccess("Post Tags deleted successfully");
        this.onNavBack();
      })
      .catch((oError: any) => {
        oViewModel.setProperty("/busy", false);
