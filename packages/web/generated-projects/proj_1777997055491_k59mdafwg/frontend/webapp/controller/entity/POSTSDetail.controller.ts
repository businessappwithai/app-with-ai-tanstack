/**
 * POSTS Detail Controller - Object Page (Column 3)
 *
 * Dedicated detail controller for Posts entity.
 ...
 */
export default class POSTSDetail extends Controller {
  ...
  public onInit(): void {
    ...
    this._getRouter().getRoute("pOSTSDetail").attachPatternMatched(this._onRouteMatched.bind(this));
  }
    ...
    try {
      oModel
        ...
        .then(() => {
          oViewModel.setProperty("/busy", false);
          oViewModel.setProperty("/editable", false);
          MessageHelper.showSuccess("Posts saved successfully");
        })
        .catch((oError: any) => {
          oViewModel.setProperty("/busy", false);
  ...
  public onDeletePress(): void {
    MessageBox.confirm("Are you sure you want to delete this Posts?", {
  ...
  private _deleteRecord(): void {
    ...
    oModel
      ...
      .then(() => {
        oViewModel.setProperty("/busy", false);
        MessageHelper.showSuccess("Posts deleted successfully");
        this.onNavBack();
      })
      .catch((oError: any) => {
        oViewModel.setProperty("/busy", false);
