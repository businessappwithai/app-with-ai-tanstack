/**
 * TAGS Detail Controller - Object Page (Column 3)
 *
 * Dedicated detail controller for Tags entity.
 ...
 */
export default class TAGSDetail extends Controller {
  ...
  public onInit(): void {
    ...
    this._getRouter().getRoute("tAGSDetail").attachPatternMatched(this._onRouteMatched.bind(this));
  }
    ...
    try {
      oModel
        ...
        .then(() => {
          oViewModel.setProperty("/busy", false);
          oViewModel.setProperty("/editable", false);
          MessageHelper.showSuccess("Tags saved successfully");
        })
        .catch((oError: any) => {
          oViewModel.setProperty("/busy", false);
  ...
  public onDeletePress(): void {
    MessageBox.confirm("Are you sure you want to delete this Tags?", {
  ...
  private _deleteRecord(): void {
    ...
    oModel
      ...
      .then(() => {
        oViewModel.setProperty("/busy", false);
        MessageHelper.showSuccess("Tags deleted successfully");
        this.onNavBack();
      })
      .catch((oError: any) => {
        oViewModel.setProperty("/busy", false);
